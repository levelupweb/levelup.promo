...
class ContactForm extends React.Component {
		...
		if (error.response) {
			if (error.response.status === 422) {
				return this.handleValidationErrors(
					Object.values(error.response.data.errors).map(err => err.msg)
				);
			}
	...
	renderSubmit() {
		return (
			<button className="button ghost" onClick={this.submit}>
				Отправить
			</button>
		);
	}
	...
	render() {
		...
		return (
			<form className={isFluid && "fluid"} ref={this.handleFormRef}>
				{this.renderErrors()}
				{this.renderFields()}
				<p className="disclaimer">
					Любые персональные данные, которые вы оставляете в данной форме,
					регулируются нашей{" "}
					<a href="/politics" download target="_blank">
